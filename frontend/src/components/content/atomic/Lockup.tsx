  ...
  return (
    <span key={lockup.id} className={`content-lockup relative flex flex-row justify-center items-center p-1 bg-background dark:bg-background-dark ${textAlign} ${styles.lockup}`}>
      <span className='inline-block'>
        {leadin && <span className={`lockup-leadin block relative tracking-wide my-1 ${styles.leadin}`}>{leadin}</span>}
        <h2 className={`lockup-title block relative font-semibold my-1 ${styles.title}`}>{title}</h2>
        {subtitle && <span className={`lockup-subtitle block relative my-1 ${styles.subtitle}`}>{subtitle}</span>}
        {price && <span className={`lockup-price block relative my-1 ${styles.price}`}>{price}</span>}
